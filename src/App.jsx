...
function App() {
  ...
  const pollValidate = async (payload) => {
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      setProgress(`🔁 Checking for deposit... Attempt ${attempt}/${MAX_ATTEMPTS}`);
      ...
      try {
        ...
        if (attempt === MAX_ATTEMPTS) {
          return { success: false, message: data.error || "Timed out waiting for token deposit." };
        }
      } catch (err) {
        console.error("Polling error:", err);
        return { success: false, message: "Server error during polling." };
      }

      await new Promise((resolve) => setTimeout(resolve, POLL_DELAY_MS));
    }

    return { success: false, message: "Token transfer not detected." };
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await pollValidate(payload);
  ...
  return (
    ...
    >
      ...
      {progress && (
        <div style={{ marginTop: 20, padding: 10, color: "#555" }}>
          {progress}
        </div>
      )}
