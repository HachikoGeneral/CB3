...
async function normalizeAmount(amount, decimals) {
  // Convert amount like "100.5" to lamports BigInt (e.g., 100.5 * 10^decimals)
  return BigInt(Math.round(parseFloat(amount) * 10 ** decimals));
}

// Check if sender sent expectedLamports of tokenMint to vaultTokenAccount
  ...
  for (const sigInfo of signatures) {
    ...
    for (const ix of instructions) {
      if (
        ...
        // Found matching transfer from sender to vault for expected amount
        return true;
      }
    }
  }
...
app.post("/api/validate-tokens", async (req, res) => {
  try {
    ...
    // Check deposits from sender by scanning recent tx history
