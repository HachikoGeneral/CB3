...
// Converts human-readable amount string into lamports (BigInt)
async function normalizeAmount(amount, decimals) {
  return BigInt(Math.round(parseFloat(amount) * 10 ** decimals));
}

// Validates if a sender sent a specific amount of tokenMint to vaultTokenAccount
async function checkSenderDeposit({
  ...
  const signatures = await connection.getSignaturesForAddress(vaultTokenAccount, { limit: 20 });
  ...
  for (const sigInfo of signatures) {
    ...
    for (const ix of instructions) {
      if (
        ...
        console.log(`✅ Found valid transfer in tx: ${sigInfo.signature}`);
        return true;
      }
    }
  }

  console.log(`❌ No matching transfer from ${senderPubkey.toBase58()} to ${vaultTokenAccount.toBase58()}`);
  return false;
}
